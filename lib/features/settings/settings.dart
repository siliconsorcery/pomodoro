...
class _SettingsDialogState extends State<SettingsDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text("Settings"),
      contentPadding: const EdgeInsets.symmetric(horizontal: 26.0, vertical: 8.0),
      content: Container(
...
class _NumericInput extends StatefulWidget {
  ...
  _NumericInput({required this.controller, required this.label, required this.valueChanged, Key? key})
      : super(key: key);
          ...
          children: [
                    ...
                    if (int.tryParse(value) == null) {
                      setState(() {
                        widget.controller.value = TextEditingValue(
                          text: _enteredText,
                          selection: TextSelection.collapsed(offset: _enteredText.length),
                        );
                      });
                    }
            ...
            Column(
              children: [
                InkWell(
                  onTap: () {
                    var incrementVal = (int.parse((widget.controller.text)) + 1).toString();
                ...
                InkWell(
                  onTap: () {
                    var decrementVal = (max(1, int.parse((widget.controller.text)) - 1)).toString();
